{
  TString thename = "/home/abl/impulseResponseCode/A3ImpulseResponse/transferFunctions/fullTF_";
    ...
    for(int tmb = 0; tmb < 3; tmb++)
    {
      for(int pol = 0; pol < 2; pol++)
      {
        ...
        if(pol == 0)
        {
          ...
          if(tmb == 0 && phi == 3)
          {
            gH[cH]->SetLineColor(kBlue);
            gH[cH]->SetLineWidth(2);
          }
          if(tmb == 2 && phi == 13)
          {
            gH[cH]->SetLineColor(kRed);
            gH[cH]->SetLineWidth(2);
          }
          cH++;
        }
...
//  gH[0]->GetYaxis()->SetRangeUser(-.09,.09);
  gH[0]->GetXaxis()->SetRangeUser(0,50);
  gH[0]->Draw("alp");
...
//  gV[0]->GetYaxis()->SetRangeUser(-.09,.09);
  gV[0]->GetXaxis()->SetRangeUser(0,50);
